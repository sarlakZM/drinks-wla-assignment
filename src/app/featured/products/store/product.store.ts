...
import { ProductDetialsModel, ProductListModel } from '../models/product.model';
...
export const ProductsStore = signalStore(
  ...
  withComputed(({ products, productDetails }) => ({
    ...
    selectedProductDetailsImage: computed(
      () => productDetails().drinks.at(0)?.strDrinkThumb
    ),
    selectedProductDetailsDescription: computed(
      () => productDetails().drinks.at(0)?.strDrink
    ),
    selectedProductDetailsTags: computed(
      () => productDetails().drinks.at(0)?.strTags
    ),
    ...
    selectedProductDetailsIngredients: computed(() => {
      const details = productDetails().drinks.at(0) as ProductDetialsModel;
      const ingredients =
        details &&
        Object.entries(details)
          .filter(
            ([key, value]) => key && value && key.includes('strIngredient')
          )
          .map(row => row.at(1));
      return ingredients ?? [];
    }),
